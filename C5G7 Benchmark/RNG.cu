...
// half open: (0, 1]
__host__ __device__ double GnuAMCM::uniform_right_closed(double lowerLimit, double upperLimit) {
	unsigned long long xi_nplus1 = this->gen();
	return lowerLimit + (xi_nplus1) * (upperLimit - lowerLimit) / (1ULL << 48);
}

__host__ __device__ double GnuAMCM::uniform_left_closed(double lowerLimit, double upperLimit) {
	unsigned long long xi_nplus1 = this->gen();
	return 1.0 - this->uniform_right_closed(lowerLimit, upperLimit);
}
